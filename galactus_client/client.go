...
func (gc *GalactusClient) Login() (*LoginResponse, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("error creating Galactus request: %w", err)
	}
	req.Header.Add("Content-Type", "application/json")

	res, err := client.Do(req)
	if err != nil {
		return nil, fmt.Errorf("error performing HTTP request to Galactus: %w", err)
	}
	defer res.Body.Close()

	body, err := io.ReadAll(res.Body)
	if err != nil {
		return nil, fmt.Errorf("error reading HTTP response from Galactus: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("error unmarshalling response from Galactus: %w", err)
	}
